...
class ArticleDetail(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if not article.sound or article.sound == 'record_none.mp3':
            ...
            # # https://ru.stackoverflow.com/questions/1106458/django-%D0%9F%D0%B5%D1%80%D0%B5%D0%B4%D0%B0%D1%87%D0%B0-%D1%84%D0%B0%D0%B9%D0%BB%D0%B0-%D0%BF%D0%BE%D0%BB%D1%8C%D0%B7%D0%BE%D0%B2%D0%B0%D1%82%D0%B5%D0%BB%D1%8E
            # FileResponse(open('media/media/'+str(article.id)+'.mp3', 'rb'))
...
class ArticleToSoundThread:
    """
        path_dir-директория сохранения файлов
        format_file - формат сохранения аудиофайла
        voice - голос оратора, досупные голоса узнать: spd-say -o rhvoice -L
        rate - скорость произношения 50-200
        pith - высота звука 50-200
        volume - сила звука 25-200
    """
    ...
    def get_sound_data(self):
        if not hasattr(self, 'path_dir'): self.path_dir = 'media'
        if not hasattr(self, 'voice'): self.voice = 'Anna'
        if not hasattr(self, 'rate'): self.rate = '85'
        if not hasattr(self, 'pith'): self.pith = '85'
        if not hasattr(self, 'volume'): self.volume = '200'
    ...
    def get_voice_from_text(self):
        ...
        args = ['RHVoice-test', "-p", self.voice, "-v", self.volume, "-t", self.pith, "-r", self.rate, "-o",
                self.path_dir + str(self.object.sound.url)]
        # Запускаем командную строку
        p = subprocess.Popen(args=args, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                             close_fds=True, shell=False)
        try:
            stdout, stderr = p.communicate(text.encode('utf-8'), timeout=30)
        except TimeoutError:
            p.kill()
    ...
    #         text = ArticleToSoundThread.get_article_text(self)
    #         tts = TTS(threads=1)
    #         tts.to_file(filename='media' + str(self.object.sound.url), text=text, voice='Anna', format_='mp3')
    #
    #     except KeyError:
